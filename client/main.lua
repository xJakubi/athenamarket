...
-- When /market is executed, show the market UI and trigger the refresh immediately
RegisterCommand('market', function(source, args, rawCommand)
    -- Show the market UI
    SetNuiFocus(true, true)
    SendNUIMessage({ action = "show" })

    -- Trigger a request to fetch and refresh market listings
    TriggerServerEvent('market:getMarketListings')  -- Request current market listings from the server
end, false)
...
-- NUI callback: Handle refreshing the market listings
RegisterNUICallback('getMarketListings', function(data, cb)
    -- Trigger server event to fetch current market listings
    TriggerServerEvent('market:getMarketListings')  -- Fetch current listings from the server
    cb('ok')
end)
